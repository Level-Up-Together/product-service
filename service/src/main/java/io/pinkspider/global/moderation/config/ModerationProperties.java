 ...
 * 이미지 검증 관련 설정 프로퍼티
 *
 * <p>설정 예시: moderation: image: provider: none # none, onnx-nsfw, aws-rekognition min-confidence:
 * 80.0 blocked-categories: - Explicit Nudity - Violence - Visually Disturbing onnx: model-path:
 * classpath:models/nsfw.onnx nsfw-threshold: 0.8
 */
@Getter
@Setter
@Configuration
...
public class ModerationProperties {
    ...
    /**
     * 검증 제공자 (none, onnx-nsfw, aws-rekognition) - none: 검증 비활성화 (기본값) - onnx-nsfw: ONNX NSFW 모델 사용
     * (비용 절감) - aws-rekognition: AWS Rekognition 사용
     */
    private String provider = "none";

    /** 최소 신뢰도 임계값 (0.0 ~ 100.0) 이 값 이상의 신뢰도로 감지된 레이블만 차단 대상으로 처리 */
    private double minConfidence = 80.0;

    /** 차단할 카테고리 목록 AWS Rekognition 기준 카테고리명 사용 */
    ...
    public static class OnnxConfig {
        ...
        /** NSFW 판정 임계값 (0.0 ~ 1.0) 이 값 이상이면 부적절한 이미지로 판정 */
