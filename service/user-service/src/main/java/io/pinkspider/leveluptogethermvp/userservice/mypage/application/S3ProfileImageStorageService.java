...
public class S3ProfileImageStorageService implements ProfileImageStorageService {
    ...
    public String store(MultipartFile file, String userId) {
        if (file == null || file.isEmpty()) {
            throw new CustomException("PROFILE_001", "error.image.empty");
        }

        if (!isValidImage(file)) {
            throw new CustomException("PROFILE_002", "error.image.invalid");
        }
        ...
        try {
            ...
            String key = "profile/" + userId + "/" + newFilename;
            ...
            log.info("프로필 이미지 S3 저장: userId={}, key={}", userId, key);
            return cdnUrl;

        } catch (IOException e) {
            log.error("프로필 이미지 S3 저장 실패: userId={}", userId, e);
            throw new CustomException("PROFILE_003", "error.image.save_failed");
        }
    }
        ...
        if (imageUrl.startsWith(s3Properties.getCdnBaseUrl())) {
            ...
            try {
                s3Client.deleteObject(DeleteObjectRequest.builder()
                        ...
                        .key(key)
                        .build());
                log.info("프로필 이미지 S3 삭제: key={}", key);
            } catch (Exception e) {
                log.warn("프로필 이미지 S3 삭제 실패: key={}", key, e);
            }
            return;
        }

        // 로컬 URL("/uploads/") 또는 외부 URL(OAuth)은 무시
