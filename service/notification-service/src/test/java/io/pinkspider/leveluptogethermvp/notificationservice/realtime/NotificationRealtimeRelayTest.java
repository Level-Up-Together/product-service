...
class NotificationRealtimeRelayTest {
    ...
    @BeforeEach
    void setUp() {
        relay = new NotificationRealtimeRelay(messagingTemplate, new ObjectMapper());
    }
    ...
    @Test
    @DisplayName("수신한 메시지를 해당 유저의 user destination으로 릴레이한다")
    void onMessage_relaysToUserDestination() {
        // given
        String payload = "{\"user_id\":\"user-1\",\"notification\":{\"id\":1,\"title\":\"t\"}}";
    ...
    @Test
    @DisplayName("잘못된 JSON이어도 예외를 전파하지 않는다")
